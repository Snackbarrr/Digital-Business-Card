  ...
  return (
    <div className="rounded-2xl border border-gray-200 p-4 md:p-5">
      ...
      {options?.values?.length ? (
        <div className="mt-4">
          <label className="block text-sm text-gray-700 mb-2">{options.label}</label>
          <select
            value={opt}
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        ...
        <Link
          href={`/checkout?sku=${encodeURIComponent(slug)}&qty=${qty}${opt ? `&opt=${encodeURIComponent(opt)}` : ''}`}
          className="rounded-xl border px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-50 text-center"
        >
          Buy now
        </Link>
      </div>
